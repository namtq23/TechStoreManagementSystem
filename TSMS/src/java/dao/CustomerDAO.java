...
// ✅ Cập nhật đầy đủ thông tin khách hàng
    ...
    public List<CustomerDTO> getCustomersByCreatedBy(String dbName, int offset, int limit, String genderFilter, int createdBy) throws SQLException {
        List<CustomerDTO> customers = new ArrayList<>();

        StringBuilder sql = new StringBuilder("""
        SELECT c.CustomerID, c.FullName, c.Gender, c.PhoneNumber, c.Email, c.Address, c.CreatedAt,
               SUM(o.GrandTotal) AS TotalSpent
        FROM Customers c
    ...
    """);
        ...
        if (genderFilter != null) {
            if ("male".equalsIgnoreCase(genderFilter)) {
                sql.append(" AND c.Gender = 1");
            } else if ("female".equalsIgnoreCase(genderFilter)) {
                sql.append(" AND c.Gender = 0");
            }
        }

        sql.append("""
        GROUP BY c.CustomerID, c.FullName, c.Gender, c.PhoneNumber, c.Email, c.Address, c.CreatedAt
    ...
    """);
        ...
        try (Connection conn = DBUtil.getConnectionTo(dbName); PreparedStatement stmt = conn.prepareStatement(sql.toString())) {
            ...
            while (rs.next()) {
                CustomerDTO customer = extractCustomerDTO(rs);
                customer.setGrandTotal(rs.getDouble("TotalSpent"));
                customers.add(customer);
            }
        }
    ...
    public int countCustomersByCreatedBy(String dbName, String genderFilter, int createdBy) throws SQLException {
        int count = 0;
    ...
    private CustomerDTO extractCustomerDTO(ResultSet rs) throws SQLException {
        CustomerDTO c = new CustomerDTO();
        c.setCustomerId(rs.getInt("CustomerID"));
        c.setFullName(rs.getString("FullName"));
        c.setGender(rs.getObject("Gender") != null ? rs.getBoolean("Gender") : null);

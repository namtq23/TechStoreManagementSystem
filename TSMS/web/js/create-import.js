...
function saveInvoice() {
    ...
    .then(data => {
        if (data.success) {
            ...
            showNotification(data.message || 'Có lỗi xảy ra khi lưu phiếu nhập', 'error');
        }
    })
    .catch(error => {
    ...
    if (confirm('Bạn có chắc muốn hoàn thành phiếu nhập này?')) {
        ...
        .then(data => {
            if (data.success) {
                ...
                setTimeout(() => {
                    window.location.href = 'wh-import';
                }, 2000);
            } else {
                showNotification(data.message || 'Có lỗi xảy ra khi hoàn thành phiếu nhập', 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            showNotification('Có lỗi xảy ra khi hoàn thành phiếu nhập', 'error');
        });
    }
}

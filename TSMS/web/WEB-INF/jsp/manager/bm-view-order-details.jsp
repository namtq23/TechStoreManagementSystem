...
<html>
    <head>
        <title>Chi tiết đơn hàng - TSMS</title>
    ...
    <body>
        ...
        <div class="main-container">
            <!-- Sidebar Filter -->
            <aside class="sidebar">
                <form method="GET" action="view-order-details" class="filter-form">
            ...
            <main class="main-content">
                ...
                <div class="page-header">
                    <div>
                        <h1>Chi tiết đơn
                            <c:choose>
                                <c:when test="${orderInfo.movementType eq 'export'}">xuất</c:when>
                                <c:otherwise>nhập</c:otherwise>
                            </c:choose>
                            hàng #${orderInfo.movementID}
                        </h1>
                    ...
                    <div class="header-actions">
                        <button class="btn btn-secondary" onclick="window.location.href = 'bm-incoming-orders'">
                            <i class="fas fa-arrow-left"></i>
                            Quay lại
                        </button>
                        <c:if test="${orderInfo.responseStatus eq 'transfer'}">
                            <button class="btn btn-primary" onclick="window.location.href = 'bm-receive-details?id=${movementID}'">
                                <i class="fas fa-clipboard-check"></i>
                                Kiểm tra để nhận hàng
                            </button>
                        </c:if>
                    </div>
                </div>
                ...
                <div class="movement-info-card">
                    <div class="info-grid">
                        <div class="info-item">
                            ...
                            <span class="movement-type-badge ${orderInfo.movementType}">
                                <c:choose>
                                    <c:when test="${orderInfo.movementType eq 'import'}">
                                        <i class="fas fa-arrow-down"></i> Nhập hàng
                                    </c:when>
                                    <c:when test="${orderInfo.movementType eq 'export'}">
                                        <i class="fas fa-arrow-up"></i> Xuất hàng
                                    </c:when>
                                    <c:otherwise>
                                        ${orderInfo.movementType}
                                    </c:otherwise>
                                </c:choose>
                            </span>
                        </div>
                        ...
                        <div class="info-item">
                            ...
                            <span class="status-badge ${orderInfo.responseStatus}">
                                <c:choose>
                                    ...
                                    <c:when test="${orderInfo.responseStatus eq 'cancelled'}">
                    ...
                    <div class="pagination">
                        ...
                        <c:choose>
                            <c:when test="${currentPage > 1}">
                                <a href="view-order-details?id=${movementID}&productFilter=${productFilter}&status=${status}&itemsPerPage=${itemsPerPage}&page=1" class="page-btn">⏮</a>
                            </c:when>
                            <c:otherwise>
                        ...
                        <c:choose>
                            <c:when test="${currentPage > 1}">
                                <a href="view-order-details?id=${movementID}&productFilter=${productFilter}&status=${status}&itemsPerPage=${itemsPerPage}&page=${currentPage - 1}" class="page-btn">◀</a>
                            </c:when>
                            <c:otherwise>
                                <span class="page-btn disabled">◀</span>
                            </c:otherwise>
                        ...
                        <c:forEach var="i" begin="${currentPage - 2 > 0 ? currentPage - 2 : 1}"
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="view-order-details?id=${movementID}&productFilter=${productFilter}&status=${status}&itemsPerPage=${itemsPerPage}&page=${i}" class="page-btn">${i}</a>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        ...
                        <c:choose>
                            <c:when test="${currentPage < totalPages}">
                                <a href="view-order-details?id=${movementID}&productFilter=${productFilter}&status=${status}&itemsPerPage=${itemsPerPage}&page=${totalPages}" class="page-btn">⏭</a>
                            </c:when>
                            <c:otherwise>
                                <span class="page-btn disabled">⏭</span>
                            </c:otherwise>
        ...
        <script>
            ...
            function resetFilters() {
                window.location.href = 'view-order-details?id=${movementID}';
            }
        ...
        <style>
            ...
            .movement-type-badge.import {
                background-color: #d4edda;
                color: #155724;
            }


            .movement-type-badge.export {
                background-color: #fff3cd;
                color: #856404;
            }
